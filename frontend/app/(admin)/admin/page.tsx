                ...
                return (
                  <div key={s}>
                    <div className="flex justify-between text-xs mb-1">
                      <span className="text-slate-600">{tos(s)}</span>
            ...
            {rev.length === 0 ? (
              ...
              rev.slice(-8).map((r) => (
                <div key={r.month} className="flex-1 flex flex-col items-center gap-1.5">
                  <div className="text-[10px] font-semibold text-navy">{money(r.total)}</div>
                  <div
                    className="w-full bg-primary/80 rounded-t-md"
                    style={{ height: `${Math.max((r.total / maxRev) * 120, 6)}px` }}
                  />
                  <div className="text-[10px] text-slate-400">{r.month.slice(5)}</div>
                </div>
              ))
            )}
          </div>
