...
function SearchInner() {
  ...
  const totalCount = orders.length + products.length + tickets.length + users.length;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          {tr
            ? 'Siparişler, ürünler, talepler ve kullanıcılar arasında ara.'
            : 'Search across orders, products, tickets and users.'}
        </p>
      </div>
            ...
            >
              {users.map((u) => (
                <ResultRow
                  ...
                  meta={[u.fullName ? u.email : undefined, roleLabel(u.role, tr)]}
                />
              ))}
            </ResultGroup>
          )}
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 px-5 py-16 text-center dark:bg-slate-900 dark:border-slate-800">
      ...
      <p className="text-sm text-slate-400 dark:text-slate-500 mt-1 max-w-sm mx-auto">
        {tr
          ? 'Sipariş, ürün, talep veya kullanıcı bulmak için yukarıya bir terim yazın.'
          : 'Type a term above to find an order, product, ticket or user.'}
      </p>
    </div>
  );
}
...
function roleLabel(r: string, tr: boolean) {
  const map: Record<string, [string, string]> = {
