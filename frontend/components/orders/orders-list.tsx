      ...
      <div className="flex items-center gap-3">
        ...
        <div className="inline-flex rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden text-sm">
          <button
            ...
            className={`px-4 py-2 ${!archived ? 'bg-primary text-white' : 'bg-white dark:bg-slate-950 text-slate-500 dark:text-slate-400'}`}
          >
            {t('active')}
          </button>
          <button
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
            {rows.map((o) => (
              <Link
                key={o.id}
                href={`${basePath}/${o.id}`}
                className="grid grid-cols-2 md:grid-cols-12 gap-3 px-5 py-3.5 items-center hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors"
              >
                <div className="md:col-span-3">
                  <div className="font-semibold text-navy dark:text-white text-sm">{o.orderNumber}</div>
                  <div className="text-[11px] text-slate-400 dark:text-slate-500">{shortDate(o.createdAt)}</div>
                </div>
                <div className="md:col-span-2 text-sm text-slate-500 dark:text-slate-400">{o.category}</div>
                <div className="md:col-span-3 text-sm text-slate-600 dark:text-slate-300 truncate">
                  {o.clientName ?? o.user?.email ?? '—'}
                </div>
                <div className="md:col-span-2 text-right font-semibold text-navy dark:text-white text-sm">
                  {money(o.total)}
                </div>
